  ...
  updateChart() {
    ...
    if (userGrowthData.length > 0) {
      ...
      if (standardData) {
        ...
        if (height < standardData.p3) {
          status = 'abnormal';
          statusText = '异常';
        } else if (heighti >= standardData.p3 && height < (currentStandard === 'WHO' ? standardData.p15 : standardData.p10)) {
          status = 'below-average';
          statusText = '中等偏下';
        } else if (height >= (currentStandard === 'WHO' ? standardData.p15 : standardData.p10) &&
          height <= (currentStandard === 'WHO' ? standardData.p85 : standardData.p90)) {
          status = 'average';
          statusText = '中等';
        } else if (height > (currentStandard === 'WHO' ? standardData.p85 : standardData.p90) &&
          height <= standardData.p97) {
          status = 'above-average';
          statusText = '中等偏上';
        } else if (height > standardData.p97) {
          status = 'exceptional';
          statusText = '超常';
        }
